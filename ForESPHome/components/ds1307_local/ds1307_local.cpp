...
namespace esphome {
namespace ds1307_local {

static const char *const TAG = "ds1307_local";

void DS1307LocalComponent::setup() {
  if (!this->read_rtc_()) {
    this->mark_failed();
  }
}

void DS1307LocalComponent::update() { this->read_time(); }

void DS1307LocalComponent::dump_config() {
  ESP_LOGCONFIG(TAG, "DS1307 (local time):");
  LOG_I2C_DEVICE(this);
  if (this->is_failed()) {
...
void DS1307LocalComponent::read_time() {
  if (!this->read_rtc_()) {
    return;
  }
  if (ds1307_.reg.ch) {
  ...
  // Часы хранят ЛОКАЛЬНОЕ время: пересчитываем поля в UTC-epoch с учётом часового пояса.
  rtc_time.recalc_timestamp_local();
...
void DS1307LocalComponent::write_time() {
  // Записываем в RTC ЛОКАЛЬНОЕ время (а не UTC, как встроенный ds1307).
...
bool DS1307LocalComponent::read_rtc_() {
  ...
  ESP_LOGD(TAG, "Read  %0u%0u:%0u%0u:%0u%0u 20%0u%0u-%0u%0u-%0u%0u  CH:%s RS:%0u SQWE:%s OUT:%s",
           ds1307_.reg.hour_10, ds1307_.reg.hour, ds1307_.reg.minute_10, ds1307_.reg.minute,
           ds1307_.reg.second_10, ds1307_.reg.second, ds1307_.reg.year_10, ds1307_.reg.year,
           ds1307_.reg.month_10, ds1307_.reg.month, ds1307_.reg.day_10, ds1307_.reg.day,
           ONOFF(ds1307_.reg.ch), ds1307_.reg.rs, ONOFF(ds1307_.reg.sqwe), ONOFF(ds1307_.reg.out));

  return true;
}

bool DS1307LocalComponent::write_rtc_() {
